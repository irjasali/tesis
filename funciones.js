...
function Encontrar() {
	...
	if($('#findWord').val().length > 0){//si el campo de pista no esta vacio
		...
		for (var i=0; i <= palabras.length-1; i++) {//hago un recorrido del arreglo de palabras
			...
			if(temp.endsWith(",") || temp.endsWith(".") || temp.endsWith(";") ){//si la palabra temporal acaba en punto o coma, hay que quitarsela para procesar la palabra
			...
			// si esta distancia es menor que la siguiente distancia más corta encontrada, o si la palabra más corta siguiente aun no se ha encontrado
			if (lev <= shortest || shortest < 0) {// establece la coincidencia más cercana y la distancia más corta
				matchlev = pista;
				shortest = lev;
				sm = palabra;
			...
			if(shortest === 0)//si encontre la palabra exacta le digo que encontre el patron exacto
			    $("#aproximacion").text("Patrón exacto encontrado");
